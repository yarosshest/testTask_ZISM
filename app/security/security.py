...
async def authenticate_user(username: str, password: str, db: Db = Depends(db_ins)) -> User | bool:
...
async def get_current_user(token: Annotated[str, Depends(oauth2_scheme)], db: Db = Depends(db_ins)) -> User | None:
    if token is None:
        return None
    try:
